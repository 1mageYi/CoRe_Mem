...
class SearchPipeline:
    ...
    @staticmethod
    def _core_threshold(centrality: dict[str, float]) -> float:
        if not centrality:
            return 0.0
        vals = sorted(centrality.values(), reverse=True)
        top_n = max(1, int(len(vals) * 0.2))
        return vals[top_n - 1]
