...
- `TD-054` `[doing]` 以 `v6.5 Facetized Observation-to-Memory Redesign` 为目标，把 clause-level observation / slot memory 升级成 facet-structured latent memory unit。
  ...
  - 当前 partial：`latest_stage2_v65_facetized_memory_eval.json` 记录 authoritative facetizer、`8` 类 facet schema、`facet_candidate_count = 4834`、`facet_rerouted_actions = 1014`；`latest_stage2_v65_persistent_state.json` 记录 non-collapse `core=691 / residual=1732 / writes=3776`
  - 当前负结果：`latest_stage2_v65_error_attribution.json` 记录 `needed_facet_missing_count = 434 > parser_only 409`、`facet_written_but_reader_missed_count = 5`；`latest_stage2_v65_personamem_no_routing.json` 记录 full589 no-routing `150/589`，text-only `234/589`，option-only `235/589`
  - 最新 discard：`research-results.tsv` iteration `2 discard` 说明 cue-anchored phrase retention 只能把 `needed_facet_missing` 改善到 `448`、把 no-routing 改善到 `138/589`，但 verifier 仍停在 `85`，因此该 trial 已回滚，不能记成 retained keep
  - 最新 refine：`research-results.tsv` iteration `3 refine` 说明 facet-type glossing + salient phrase selection 已把 selected slots 明显清理干净，并把 `needed_facet_missing` 压到 `434`、把 no-routing 提到 `150/589`；当前 latest restore 已回到这条 refine truth，但 retained metric 仍是 `85`
  - 最新 discard：`research-results.tsv` iteration `4 discard` 说明 decision features 直接吃 `selected slot canonical_gloss` 并不能把当前 refine line 再推高：full589 仍是 `150/589`，verifier 仍是 `85`
  - 最新 discard：`research-results.tsv` iteration `5 discard` 说明 reader facet-metadata alignment 会把 full589 直接打回 `128/589`，并把 `needed_facet_missing` 恶化到 `456`
  - 最新 discard：`research-results.tsv` iteration `7 discard` 说明 option-conditioned decision readout 会把 full589 no-routing 从 `150/589` 打回 `142/589`，并把 `needed_facet_missing` 恶化到 `441`；当前 code 和 latest artifacts 已按 rollback policy restore 回 refine line
  - 最新 discard：`research-results.tsv` iteration `8 discard` 说明 intent-conditioned facet readout 把 query-intent facet priors 与 duplicate-facet crowding penalty 接进 reader shortlist 后，full589 仍会回退到 `147/589`，并把 `needed_facet_missing` 恶化到 `436`
  - 最新 pivot/search：`research-results.tsv` iterations `9 pivot` 与 `10 search` 已正式关闭 “更广 supervision 但仍直接改 slot-selection heuristic” 的子 family，并记录 DST-as-reading-comprehension 与 MMR 两条主来源启发
  - 当前下一步：当前 local feature surgery family 已通过 iteration `6 pivot` 正式关闭，而 iteration `7-8 discard` 又排除了简单的 per-option reread 和 intent-conditioned slot reranking。下一步必须直接做 question-type / facet-type supervision 或 relevance+novelty 的 diversified shortlist，而不是继续停留在 token-level canonicalization / dedup、单条 feature 接线、naive option-conditioned reread 或 hand-tuned slot-selection priors
...
- `TD-051` `[doing]` 以 `v6.2 Learned Write-Worthiness / Attribute-Validity Before Extraction` 为目标，把 v6.1 的 persistent + learned reader/decision 主链升级成真正由 learned write-quality 控制输入质量的 memory system。
  ...
  - 当前 verifier：[scripts/verify_stage2_v62_write_quality.py](/media/storage/mingjing/workspace/CoRe_Mem/scripts/verify_stage2_v62_write_quality.py)
  - 核心目标：`raw dialogue -> learned write-worthiness / attribute-validity -> cleaner persistent banks -> learned reader -> learned decision -> answer`。
  - 硬约束：no fallback、no shortcut、no benchmark-specific heuristic、no provider prompt trick、no PersonaMem gold leakage。
  - 新增硬边界：不允许回到 answer-time routing；不允许 raw-context retrieval 作为 authoritative path；不允许 broad heuristic cleanup 冒充 write-quality gain。
...
- `TD-050` `[doing]` 以 `v6.1 Learned Reader/Decision over Persistent Memory` 为目标，把 v6 的 persistent substrate 升级为真正由 learned reader/readout 主导的 authoritative memory system。
  ...
  - 当前 partial：current HEAD 已新增 `scripts/publish_stage2_v61_learned_reader_decision.py` 与 `src/core_mem/v2/v61_learned_memory.py`，并把 verifier 推到 `85`
  - 当前 partial：authoritative learned reader / learned decision head、semantic slot compaction、typed residuals、hard internal eval、no-answer-routing 与 no-gold substrate proof 已成立；latest keep 又把 low-information slot filtering 接进 substrate，把 no-routing PersonaMem 从 `182` 拉到 `191`
  - 当前负结果：PersonaMem full589 no-routing `191/589`，text-only `180/589`，option-only `235/589`，对 text-only margin `+11`
  - 当前 soft blocker：fresh managed run 已把 narrow decision tweak、bank simplification、post-hoc fragment suppression 与 parser self-state precision 全试过；最新 parser trial 虽把 absolute no-routing 拉到 `197/589`，但 text-only 同步升到 `194/589`，verifier 仍是 `85`
  - 下一步已切到：`TD-051 / WS-037 / v6.2`，即 learned write-worthiness / attribute-validity before extraction；当前这一条线保留为 blocked baseline / handoff truth
