...
- `2026-04-21` 当前又新增一条 answer-head regression-rollback truth：当前 worktree 已撤回 `3234a4b` 给 `OptionScoringHead` 引入的 `support-density / unsupported-detail / option_length` 惩罚，让 scoring 回到 v32 的 overlap-style ranking，同时保留 `12e676a` 的 low-info token cleanup。新增 regression tests 已直接覆盖两类先前被这套惩罚压错的 case：`0d...` 的 cooking recommendation projection，以及 `2cef...` 的 preference-evolution option ordering。
- 对应 full guard 已重新通过，但 `scripts/verify_stage2_v33_longrun.py --score-only` 仍然是 `36`，所以这轮目前仍只能诚实记为 refine，而不是 retained keep。当前 in-flight `smoke8` rerun 也只完成前 `4` 条样本，不过这 `4` 条都保持正确；因此最新下一步应先等待 broader local-only measurement 收口，判断这条 rollback 是否真的把 narrow gain 外扩到更大的 Persona slice。如果没有，就继续回到剩余 projection miss 的结构性分析。
