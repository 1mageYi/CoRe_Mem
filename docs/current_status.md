...
- 测试状态：当前 stage-2 guard `pytest -q tests/test_stage2_training_runtime.py tests/test_stage2_local_eval.py tests/test_stage2_data_pipeline.py tests/test_stage2_public_data.py` 通过（18 tests）；`scripts/run_experiment.py --verify-only` 输出 `36`，`scripts/verify_stage1_acceptance.py` 返回 `5/7`，`scripts/verify_stage2_status.py --score-only` 输出 `50`，`scripts/verify_stage2_acceptance.py` 返回 `7/7`，`scripts/verify_stage2_experiment_status.py --score-only` 输出 `13`
