...
- 对应 full guard 已重新通过，但 `scripts/verify_stage2_v33_longrun.py --score-only` 仍然是 `36`，所以这轮当前仍只能诚实记为 refine，而不是 retained score gain。最新下一步因此不再是“先验证 broader local-only gain 是否存在”，而是直接分析剩余 `344ea... / 0adf...` 两条 residual projection miss：若它们仍可用 generic lexical/semantic refine 解决，就继续留在 answer-head；若 belief 本身依旧过粗，就应把问题显式上推回 belief granularity，而不是继续回头重复 parser / retrieval 微调。
