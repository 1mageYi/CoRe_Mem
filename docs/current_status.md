...
- 当前最新的恢复真相又补了一层：commit `34943df` 让 canary runner 支持 `partial provider success -> commit successes -> resume failed samples`，并为同一 `run_dir` 增加 `.active.lock`。由于旧 `outputs_v2/runs/v32_full_personamem_512/` 这条线曾被 pre-lock wrapper 并发污染，current authoritative progress 已切到新的 clean run `outputs_v2/runs/v32_full_personamem_512_clean/`；截至当前检查，它已真实推进到 `14/512`。因此当前 retained truth 仍是 `38/44`，而 full holdout 约束则继续诚实标注为 external provider throughput bound。
