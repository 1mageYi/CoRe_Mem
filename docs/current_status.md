...
- 当前 `v2.6` 的最新真实状态是：fresh baseline 虽然起点是 `stage2_v26_longrun_score = 9`，但 current-head `c8c8e19` 上的 fresh canary 与 artifact publish 已经把 verifier 推到 `24/26`，并在补齐当前状态文档 wording 后只剩真实质量缺口待突破。历史 trial HEAD `077cbf3` 已先补一轮 query-intent-aware temporal retrieval / belief scoring；本 session 随后确认 `GPT_AGENT_API_KEY=SET`，因此 `BL-007` 已被清除。新的真实瓶颈先切到 dense `other_fact` write 吞吐：sample `51a45a95` 原先会触发 `11` 次 learned slot-assignment arbitration，其中 `10` 次属于低 overlap overwrite。refine commit `a04effe` 已把 learned prompt 收缩到相关候选上下文，并为 weak `other_fact` overwrite 增加 fast-path；对应 profiling 已把 `51a45a95` 的 arbitration 次数压到 `1`，同一 partial `LongMemEval-S 128` resumed run 也已从 `2/128` 前进到 `3/128`，且 `51a45a95` 当前在 current-head 上命中 `Target`。随后 refine commit `3051b0f` 又把 learned belief / slot-assignment predictor 的重载移出 sample loop，fresh current-head `LongMemEval-S 128` run `outputs_v2/runs/20260417T055905Z_stage2_memory_canary_longmemeval/` 最终完整结束在 `provider_exact_match = 11`、`local_exact_match = 10`；这相对 retained `v2.5` 的 `10 / 10` 只形成 `provider +1 / local +0`，因此还不能诚实宣称 current-head `LongMemEval-S 128` 已明确超过 baseline。current-head `PersonaMem 128` run `outputs_v2/runs/20260417T072623Z_stage2_memory_canary_personamem/` 已完整结束在 `provider_exact_match = 44`、`local_exact_match = 33`，相对 retained `v2.5` 的 `38 / 28` 保持正向 guard。随后 publish 入口已把 current-head `v2.5` / `v2.6` aliases、`write / retrieve / belief` gain artifacts、LongMemEval analysis 与 holdout full-benchmark summary 一并刷新到 `scripts/verify_stage2_v26_longrun.py` 受控链路中；其中 `belief_gain.positive_gain = true`，但 `write` 仍是 `provider +1 / local +0`，因此当前 remaining current-head evidence 仍主要指向 `other_fact` / `projection` family，且真实剩余硬缺口只剩 `LongMemEval-S 128` local 侧尚未超过 baseline。
- 当前最值得延续的训练结论是：
