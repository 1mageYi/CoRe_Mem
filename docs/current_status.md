...
- 因此当前 `v33` 已进入新的 provider-contract blocker：若要继续 honest 追 `LongMemEval-S 500 / PersonaMem 512` learned-authoritative holdout，必须先得到用户明确批准，切换当前 provider/interface；在未获批准前，不再继续烧 full holdout API，current retained truth 保持 `36/47`
