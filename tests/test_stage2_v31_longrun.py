...
def test_v31_longrun_verifier_scores_baseline_when_docs_and_v30_retained_artifacts_exist(tmp_path: Path) -> None:
    ...
    for name, payload in {
        ...
        "latest_stage2_v30_full_holdout_baseline.json": {"holdout_only": True, "longmemeval_sample_count": 500, "personamem_sample_count": 589},
...
def test_v31_longrun_verifier_passes_with_full_latent_belief_write_and_holdout_artifacts(tmp_path: Path) -> None:
    ...
    for name, payload in {
        ...
        "latest_longmemeval_stage2_v31_full.json": {"provider_exact_match": 20, "local_exact_match": 15},
        "latest_personamem_stage2_v31_full.json": {"provider_exact_rate": 0.36, "local_exact_rate": 0.19},
    }.items():
        _write_json(repo_root / "outputs_v2" / "artifacts" / name, payload)
