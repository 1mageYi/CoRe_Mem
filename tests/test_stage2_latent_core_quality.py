...
def test_stage2_latent_core_quality_uses_latest_personamem_canary(tmp_path: Path):
    repo_root = tmp_path / "repo"
    (repo_root / "docs").mkdir(parents=True)
    (repo_root / ".agent-os").mkdir(parents=True)
    (repo_root / "outputs_v2" / "evals_local").mkdir(parents=True)
    (repo_root / "outputs_v2" / "evals_benchmark").mkdir(parents=True)
    (repo_root / "outputs_v2" / "runs" / "personamem").mkdir(parents=True)
    (repo_root / "outputs_v2" / "runs" / "longmemeval").mkdir(parents=True)
    (repo_root / "scripts").mkdir(parents=True)
    ...
    personamem_predictions = repo_root / "outputs_v2" / "runs" / "personamem" / "predictions.jsonl"
    personamem_predictions.write_text(
        "".join(
            json.dumps(
                {
                    "expected_answer": "(a)",
        ...
        json.dumps(
            {
                ...
                "status": "completed",
                "sample_count": 64,
                "predictions_path": str(personamem_predictions),
            }
        ),
        encoding="utf-8",
    )

    longmemeval_predictions = repo_root / "outputs_v2" / "runs" / "longmemeval" / "predictions.jsonl"
    longmemeval_predictions.write_text(
        "".join(
            json.dumps(
                {
...
def test_stage2_latent_core_quality_ignores_learned_personamem_canary(tmp_path: Path):
    repo_root = tmp_path / "repo"
    (repo_root / "docs").mkdir(parents=True)
    (repo_root / ".agent-os").mkdir(parents=True)
    (repo_root / "outputs_v2" / "evals_local").mkdir(parents=True)
    (repo_root / "outputs_v2" / "evals_benchmark").mkdir(parents=True)
    (repo_root / "outputs_v2" / "runs" / "symbolic").mkdir(parents=True)
    (repo_root / "outputs_v2" / "runs" / "learned").mkdir(parents=True)
    (repo_root / "scripts").mkdir(parents=True)
    ...
    symbolic_predictions = repo_root / "outputs_v2" / "runs" / "symbolic" / "predictions.jsonl"
    symbolic_predictions.write_text(
        "".join(
            json.dumps(
                {
                    "expected_answer": "(a)",
        ...
        json.dumps(
            {
                ...
                "status": "completed",
                "sample_count": 64,
                "predictions_path": str(symbolic_predictions),
            }
        ),
        encoding="utf-8",
    )

    learned_predictions = repo_root / "outputs_v2" / "runs" / "learned" / "predictions.jsonl"
    learned_predictions.write_text(
        "".join(
            json.dumps(
                {
                    "expected_answer": "(a)",
                    "provider_prediction": "(b)",
                    "memory_answer_local": "",
                }
            )
            + "\n"
        ...
        json.dumps(
            {
                ...
                "predictions_path": str(learned_predictions),
            }
        ),
        encoding="utf-8",
    )
