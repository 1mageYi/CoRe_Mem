...
def test_structured_memory_system_prefers_specific_preference_over_generic_fact_for_recommendations():
    system = StructuredMemorySystem()
    system.observe_turn(
        "I used to keep a public album review blog.",
        source_dataset="synthetic",
        source_dialogue_id="dlg-1",
        source_turn_id="turn-1",
        session_id="sess-1",
    ...
    system.observe_turn(
        "I enjoy producing music with software.",
        source_dataset="synthetic",
        source_dialogue_id="dlg-1",
        source_turn_id="turn-2",
        session_id="sess-1",
    ...
    result = system.query("query-recommend", "What would you suggest for a creatively fulfilling weekend getaway?")
    assert result.selected_slots[0].relation == "music_preference"
    assert result.belief_state.belief_items[0].relation == "music_preference"


def test_structured_memory_system_prefers_constraint_when_query_asks_for_reason_behind_change():
    system = StructuredMemorySystem()
    system.observe_turn(
        "I enjoy podcasting about music trends.",
        source_dataset="synthetic",
        source_dialogue_id="dlg-1",
        source_turn_id="turn-1",
        session_id="sess-1",
    ...
    system.observe_turn(
        "I can't stand the constant public scrutiny.",
        source_dataset="synthetic",
        source_dialogue_id="dlg-1",
        source_turn_id="turn-2",
        session_id="sess-1",
    ...
    result = system.query("query-reason", "Why did I decide I don't enjoy podcasting about music trends anymore?")
    assert result.selected_slots[0].relation == "constraint"
    assert result.belief_state.belief_items[0].relation == "constraint"
