...
def _build_split_manifest(root: Path, split: str, row_count: int = 1) -> Path:
    ...
    for index in range(row_count):
        ...
        observation = {
            ...
            "_meta": {"dataset": "sgd", "sample_id": f"slot-{suffix}", "task_name": "slot_autoencoding"},
        }
        lifecycle = {
            "memory_context": [
                {
            ...
            "_meta": {"dataset": "sgd", "sample_id": f"life-{suffix}", "task_name": "lifecycle_prediction"},
        }
        belief = {
            ...
            "_meta": {"dataset": "sgd", "sample_id": f"belief-{suffix}", "task_name": "composition_to_belief"},
        }
...
def test_build_matched_teacher_vs_silver_split_manifests_filters_to_teacher_covered_rows(tmp_path: Path) -> None:
    output_root = tmp_path / "outputs_v2"
    manifest_artifact = output_root / "artifacts" / "latest_stage2_v27_32k_manifest.json"
    split_payload = {
