...
for line in debug_path.open(encoding="utf-8"):
    ...
    if not field_names_printed:
        print("retrieval_diagnostics keys:", list(diag.keys()))
...
print(f"\n=== Semantic rank (best_semantic_rank_global) — pure semantic order in expanded pool ===")
print(f"Samples: {n}, valid: {sum(1 for r in sem_ranks if r < 9999)}")
valid_sem = sorted(r for r in sem_ranks if r < 9999)
print(f"mean={statistics.mean(valid_sem):.1f}, median={statistics.median(valid_sem):.1f}")
for cutoff in [8, 12, 16, 20, 24, 32, 40]:
    covered = sum(1 for r in valid_sem if r <= cutoff)
    print(f"  k={cutoff:2d}: coverage {covered}/{len(valid_sem)} = {covered/len(valid_sem):.1%}")

print(f"\n=== Rank after fused rerank (gold_rerank_position_among_expanded) ===")
valid_rr = sorted(rerank_ranks)
if valid_rr:
    print(f"Samples: {len(valid_rr)}")
    print(f"mean={statistics.mean(valid_rr):.1f}, median={statistics.median(valid_rr):.1f}")
    for cutoff in [8, 12, 16, 20, 24, 32, 40]:
        covered = sum(1 for r in valid_rr if r <= cutoff)
        print(f"  k={cutoff:2d}: coverage {covered}/{len(valid_rr)} = {covered/len(valid_rr):.1%}")
