...
- Top next action: `TD-054 / WS-040 / v6.5` 当前 current HEAD 已恢复到 iteration `3 refine` 这条 active baseline：facet-type glossing + salient phrase selection 已把 `needed_facet_missing` 压到 `434`、把 PersonaMem no-routing 提到 `150/589`，但 verifier 仍停在 `85`。最新 iteration `4 discard` 已进一步排除“只把 decision features 接上 selected slot canonical_gloss 就能转成 gain”这条窄修补线；下一步若继续推进，必须直接改 question-conditioned reader / decision supervision 本身，而不是继续堆局部 feature。
- Active workstreams: `WS-040`
- Active workstream label: `TD-054 / WS-040`
- Active workstream version: `v6.5`
- Current v6.5 refine truth: current HEAD `3e31e6f` 已把 materialized `canonical_gloss` 改成 `facet_type=value`，并把 `preference_target` / `generic_fact` 从“leading tokens”改成 salient phrase。`latest_stage2_v65_error_attribution.json` 当前为 `needed_facet_missing_count = 434 > parser_only 409`；`latest_stage2_v65_personamem_no_routing.json` 当前为 full589 no-routing `150/589` vs text-only `234/589` vs option-only `235/589`；当前 `selected_slot_glosses` 已不再被 relation-level boilerplate 主导，但 verifier 仍是 `85`，所以这是 active refine line，不是 retained keep。
