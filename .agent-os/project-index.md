...
- Verifier compatibility note: `TD-035 / WS-021`、`TD-036 / WS-022`、`TD-037 / WS-023`、`TD-038 / WS-024`、`TD-039 / WS-025`、`TD-040 / WS-026`、`TD-041 / WS-027`、`TD-042 / WS-028`、`TD-043 / WS-029`、`TD-044 / WS-030`、`TD-045 / WS-031`、`TD-046 / WS-032`、`TD-047 / WS-033`、`TD-048 / WS-034`、`TD-049 / WS-035`、`TD-050 / WS-036` 与 `TD-051 / WS-037` 的历史完成态/blocked/search truth 仍保留在文档与 artifact 中；当前 active 主线为 `TD-052 / WS-038 / v6.3`。
...
- `WS-038` `[doing]`: Stage-2 `v6.3 Recall-Preserving Confidence-Aware Write Policy`，目标是在保留 v6.1 reader/decision 与 v6.2 write-quality 主链的前提下，把 binary hard filter 升级成 non-collapsing memory policy。
  - Current task: `TD-052`
  ...
  - Current retained partial: current HEAD 已新增 [scripts/publish_stage2_v63_write_policy.py](/media/storage/mingjing/workspace/CoRe_Mem/scripts/publish_stage2_v63_write_policy.py) 与 [src/core_mem/v2/v63_write_policy.py](/media/storage/mingjing/workspace/CoRe_Mem/src/core_mem/v2/v63_write_policy.py)，并将 verifier 推到 `85`
  - Current retained partial: `latest_stage2_v63_write_policy_eval.json` 记录 authoritative four-way policy、`support_coverage_recall = 1.0 > disabled 0.0625`、`write_recall = 1.0 > disabled 0.0769`
  - Current retained partial: `latest_stage2_v63_persistent_state.json` 记录 non-collapse `core_bank=22`、`residual_bank=476`、`stream_observations_written=652` 与 `weak_but_keep_residual_count = 12`
  - Current negative truth: `latest_stage2_v63_personamem_no_routing.json` 记录 full589 no-routing `179/589`，text-only `180/589`，option-only `235/589`；`latest_stage2_v63_decision.json` 记录 `negative_result`
  - Latest discard: 更激进的 durable-fact core promotion 会把 bank 推到 `core=40 / residual=435`，但 full589 no-routing 掉到 `152/589`；selected-support-gloss overlap decision-feature trial 会把 full589 no-routing 从 retained line 打坏到 `173/589`；state-anchored support-supervision expansion 会打到 `144/589`；scenario-aware reader semantics 会打到 `162/589`；四条线都已被 `git revert --no-edit` 回滚
  - Current next action: [PIVOT] preserve the current non-collapse write-policy line and replace writer/readout-neighbor tweaks with question-conditioned belief routing / selection changes; do not regress to tiny clean banks, answer-time routing, or raw-context retrieval
...
- `TD-050 / WS-036 / v6.1` 当前是 active top next action。当前结构性缺口已补齐到 verifier `85`，并已把 PersonaMem full589 no-routing 从 `140` 继续推到 `191`；接下来必须继续缩小 option-only gap，而不是继续补结构分。
  - Runtime truth: fresh baseline `45`，current retained score `85`。
  - Current evidence truth: authoritative learned reader、learned decision head、semantic slot compaction、typed residuals、hard internal eval、no-answer-routing 与 no-gold substrate proof 已发布；当前 PersonaMem no-routing 为 `191/589`，已超过 text-only `180/589`，但仍低于 option-only `235/589`，negative_result 仍成立。
  - Next focus:
    - 继续提出新的 persistent-bank readout / decision-quality hypothesis，重点收敛当前仍残留的 option-only gap 与 `disabled_core_residual_split_accuracy > full_accuracy` 暴露出的 bank-quality问题，而不得回到 query-option shortcut、answer-time routing 或 raw full-context retrieval。
