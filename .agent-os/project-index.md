...
- Top next action: `TD-044 / WS-030` 当前仍是 stage-2 主线。当前 retained `v33` 仍是 `36/47 keep`；commit `2d3e59c` 已把 provider raw-output repair 接进 canary runner / resume path，而当前 Persona learned-authoritative partial full-holdout 已推进到 `99/512`，provider/local 为 `18/99`、`42/99`。最新 layered analysis 说明主矛盾已经从纯 provider-contract 污染收窄为 `projection/local = 54` 与 `provider-only = 27`；本轮后续 exploration 已进一步收敛成四层 truth：其一，support/value hygiene 线仍然不够，grounded same-relation backfill 与 relation-consistent support cleanup 都没有形成外部 keep；其二，current HEAD `3234a4b` 已把 option scorer 收紧成“过滤低信息词 + 惩罚 unsupported 长细节”的 generic ranking，clean targeted Persona `4`-sample gate `outputs_v2/v33_option_penalty_targeted_clean/` 从 `0/4` 提到 `1/4`，且没有 previously-correct 样本退化；其三，preference-facet fast-path 单独不足以形成外部 keep；其四，current HEAD `65c2612` 把 software-centric music clauses 规范成稳定 `music_preference` facet 并保留 symbolic=`new` 的 preference-facet fast-path 后，clean targeted Persona `4`-sample gate `outputs_v2/v33_music_software_facet_local/artifacts/20260420T231900Z_compare.json` 又从 retained `1/4` 提到 `2/4`，其中 `5370...` 明确翻正。下一步仍保留当前 `gpt-agent.cc/v1` 路径，但 focused hypothesis 现在应更具体地前推到 selected-slot ranking / belief relation stability，尤其是 `acd742...` 与 `a40d5...` 这类 recall/reason case，因为 software facet 已开始回流，但 top-1 unresolved slot 与 belief relation drift 仍未解。
