...
- Top next action: `TD-051 / WS-037 / v6.2` 当前主线已从 `v6.1` 的 learned reader/decision handoff 切到 learned write-worthiness / attribute-validity。现有 retained truth 是：v6.1 已把 PersonaMem full589 no-routing 提到 `191/589`，高于 text-only `180/589`，但仍低于 option-only `235/589`；因此当前最有信息量的下一步不是继续修 decision 小头，而是把 learned write-quality 接进 authoritative write path，减少低价值 observation / attribute 进入 persistent state。
