...
- Top next action: `TD-054 / WS-040 / v6.5` 当前 active line 的 latest runtime truth 已稳定到 `needed_facet_missing = 438`、PersonaMem no-routing `146/589`、verifier `85`。iteration `4 discard` 已排除 direct decision-gloss feature，iteration `5 discard` 又排除了 reader facet-metadata alignment，iteration `6 pivot` 则正式关闭了这组 local feature surgery；下一步若继续推进，必须直接改 broader question-conditioned reader / decision supervision，而不是继续堆局部 feature。
- Active workstreams: `WS-040`
- Active workstream label: `TD-054 / WS-040`
- Active workstream version: `v6.5`
- Current v6.5 refine truth: current HEAD `3e31e6f` 已把 materialized `canonical_gloss` 改成 `facet_type=value`，并把 `preference_target` / `generic_fact` 从“leading tokens”改成 salient phrase。`latest_stage2_v65_error_attribution.json` 当前为 `needed_facet_missing_count = 438 > parser_only 409`；`latest_stage2_v65_personamem_no_routing.json` 当前为 full589 no-routing `146/589` vs text-only `234/589` vs option-only `235/589`；当前 `selected_slot_glosses` 已不再被 relation-level boilerplate 主导，但 verifier 仍是 `85`，所以这是 active refine line，不是 retained keep。
