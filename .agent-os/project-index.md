...
- Top next action: `TD-044 / WS-030` 当前仍是 stage-2 主线。current-line authoritative `PersonaMem 512` full holdout 已真实完成：`outputs_v2/v33_full_personamem_512_query_overlap/evals_benchmark/20260421T001000Z_stage2_memory_canary.json` 记录 `provider/local = 182/196`，相对 retained `v32` full `183/175` 呈现 `provider -1 / local +21`，同时 provider blank/nonlabel failures 仍有 `108 / 12`。因此 `v33` 已从“只有 local smoke 共存证据”推进到“Persona full measurement completed but gain mixed”；official verifier 继续是 `36/47`，因为 current-line `LongMemEval-S 500` full artifact 仍缺，`latest_stage2_v33_full_holdout_compare.json` 还不能发布。下一步必须停止新的 local smoke，直接完成并发布 current-line `LongMemEval-S 500` authoritative full holdout，然后再决定这条 line 是继续 search 还是进入 discard/keep judgment。
...
- 推进 `TD-044 / WS-030` 的 `v3.3 / v33 learned-authoritative latent run`
  - Runtime truth: retained `v33` 仍是 `36/47 keep`；当前 working line `d48c58c` 已把 row `80 / 83 / 84 / 92 / 94 / 96` 的窄 gain 合成到同一 learned-authoritative line，并在 authoritative `smoke32` 上把旧 wider gate 从 `24/32` 推到 `29/32`
  - Current retained baseline: retained `v32` full line继续作为唯一 external compare baseline；row `99` 暴露的 3 条新 answer-projection regression `defecce... / b481... / 7028667...` 已在 authoritative `smoke4` 上被当前 patch 全部翻正，而且 control `c2d3` 继续 hold
  - Next focus:
    - 先把当前 patch widen 回旧 `smoke32` 同 manifest，验证 row `99` 的 3 条新 regression 是否会和已翻正的 `c8 / c2d3 / 2b3 / 7a4 / afd / 346 / cd81 / 0d781` 共存
    - 当前最可信的 hypothesis 是：single-support shape 上把 query overlap 关得过死，才会把 `track_full_preference_evolution` 与 `musicRecommendation recall_user_shared_facts` 这类抽象 preference belief 退回到 `(a)`
    - 只有当 widened `smoke32` 也验证这条 patch不打坏 row `99` 的 broader floor 后，才值得继续支付更重的 official measurement
