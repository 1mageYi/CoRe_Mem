...
- Top next action: `TD-051 / WS-037 / v6.2` 当前主线已完成第一轮结构 keep：authoritative write-worthiness / attribute-validity / hard write-quality eval / clean persistent-state evidence 已接进真实 write path，并把 verifier 从 `60` 提到 `85`。但 full589 no-routing 当前只有 `160/589`，低于 text-only `170/589` 与 option-only `235/589`；因此下一步必须继续提升 retained write-side recall/precision 与 cleaner-bank downstream utility，而不是回到 answer-time routing、raw-context retrieval 或 heuristic cleanup。
- Active workstreams: `WS-037`
- Active workstream label: `TD-051 / WS-037`
- Active workstream version: `v6.2`
...
- `WS-037` `[doing]`: Stage-2 `v6.2 Learned Write-Worthiness / Attribute-Validity Before Extraction`，目标是在保留 v6.1 persistent substrate、learned reader 与 learned decision 的前提下，把 authoritative write path 改成 learned write-quality 主导。
  - Current task: `TD-051`
