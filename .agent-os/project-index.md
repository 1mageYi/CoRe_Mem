...
- Top next action: `TD-044 / WS-030` 当前仍是 stage-2 主线。当前 retained `v33` 仍是 `36/47 keep`；commit `2d3e59c` 已把 provider raw-output repair 接进 canary runner / resume path，而当前 Persona learned-authoritative partial full-holdout 已推进到 `99/512`，provider/local 为 `18/99`、`42/99`。最新 layered analysis 说明主矛盾已经从纯 provider-contract 污染收窄为 `projection/local = 54` 与 `provider-only = 27`；本轮后续 exploration 已进一步收敛成十四层 truth：其一，support/value hygiene 线仍然不够，grounded same-relation backfill 与 relation-consistent support cleanup 都没有形成外部 keep；其二，current HEAD `3234a4b` 已把 option scorer 收紧成“过滤低信息词 + 惩罚 unsupported 长细节”的 generic ranking，clean targeted Persona `4`-sample gate `outputs_v2/v33_option_penalty_targeted_clean/` 从 `0/4` 提到 `1/4`，且没有 previously-correct 样本退化；其三，preference-facet fast-path 单独不足以形成外部 keep；其四，current HEAD `65c2612` 把 software-centric music clauses 规范成稳定 `music_preference` facet 并保留 symbolic=`new` 的 preference-facet fast-path 后，clean targeted Persona gate 已到 `2/4`；其五，current HEAD `2b54785` 把 `5370...` 的 relation drift 从错误 `location` 修回了 `music_preference`；其六，current HEAD `883d48d` 又新增了 generic `feedback reason` parser coverage，把 clean targeted Persona `4`-sample gate 进一步从 retained `1/4` 提到 `3/4`，其中 `a40d5...` 也已翻正；其七，current HEAD `eade2c3` 进一步放松了 same-relation latent reranking，并在 invalid relation 的 learned-belief repair 上保住已 grounded 的 concrete value，单样本 probe `outputs_v2/v33_latent_facet_rerank_local/artifacts/20260421T004426Z_acd742_compare.json` 已把 `acd742...` 从 `(b)` 翻到正确 `(c)`；其八，latest non-regression probe `outputs_v2/v33_latent_facet_rerank_local/artifacts/20260421T005228Z_hard3_compare.json` 又机械确认剩余 `5370... / a40d5... / c8a763...` 三条样本都未回退，因此 current HEAD 已把 actual hard-4 重新打到 `4/4`；其九，corrected local-only Persona `smoke8` `outputs_v2/v33_latent_facet_rerank_smoke8_localonly/evals_benchmark/20260421T010549Z_stage2_memory_canary.json` 已跑完并给出 `local_exact = 5/8`、`baseline = 1/8`，说明这轮 gain 已经开始从 hard-4 向更大的 Persona slice 外扩；其十，partial local-only `smoke16` 在主动截断前已给出 `local_exact = 6/6`、`baseline = 1/6`，进一步说明这条 gain 没有在更大 slice 上立刻塌掉；其十一，current HEAD `49e1841` 已把 boolean-like learned belief value (`true/false/yes/no`) 在 support slot canonical value 为非 boolean 时强制回填，real sample `b3588797-acdf-40d3-bcc5-951f81896f95` probe 已从 `belief=true` 修回 support-slot canonical text，targeted tests、configured guard 与 real sample replay 全部通过，但 official verifier 仍是 `36`；其十二，fresh partial `suggest_new_ideas` local-only slice `outputs_v2/v33_boolean_belief_suggest8_localonly/artifacts/20260421T021500Z_partial_summary.json` 已在主动截断前给出 `local = 4/4`、`baseline = 0/4`、`boolean-like belief rows = 0/4`；其十三，second focused `suggest_new_ideas` slice `outputs_v2/v33_boolean_belief_suggest4b_localonly/evals_benchmark/20260421T021706Z_stage2_memory_canary.json` 已完整跑完并给出 `local = 4/4`、`baseline = 0/4`，对应 boolean-check artifact `outputs_v2/v33_boolean_belief_suggest4b_localonly/artifacts/20260421T022100Z_boolean_check.json` 同样记录 `boolean-like belief rows = 0/4`；其十四，fresh mixed Persona local-only gate `outputs_v2/v33_boolean_backfill_smoke8_localonly/artifacts/20260421T023400Z_partial_summary.json` 在与旧 `eade2c3` smoke8 相同 manifest 上，主动截断前已给出 `local = 4/4`、`baseline = 1/4`、`boolean-like belief rows = 0/4`，覆盖了 `recall_user_shared_facts / provide_preference_aligned_recommendations / suggest_new_ideas / recalling_the_reasons_behind_previous_updates` 四类样本。也就是说，boolean-backfill 已经不只在 focused suggest slices 上成立，而是开始在 broader mixed gate 上体现出至少“不回退且局部更优”的信号。下一步仍保留当前 `gpt-agent.cc/v1` 路径，但 focused hypothesis 已进一步收紧成“现在最值钱的工作不是再找新的 micro-fix，而是把同一 mixed Persona local-only gate 继续往后推进，判断 current HEAD 是否有机会把旧 `5/8` 再往上推；若后续 mixed rows 不再改善，再把主火力打回 selected-slot ranking”。
...
- Active blockers: `TD-044 / WS-030` 当前已不再是“必须先切 provider/interface 才能继续”的硬 blocker；current HEAD `2d3e59c` 证明在同一 `gpt-agent.cc/v1` 代理下，把 stored `provider_raw_prediction` 重新做协议层 `<think>` / fenced-reasoning 清洗，能把 Persona partial learned-authoritative holdout 从 `provider exact = 5/66` 回收到 `15/77`，而后续扩大到 `99/512` 时 provider/local 为 `18/99`、`42/99`。最新 analysis artifact 已把剩余失败拆成 `projection = 54`、`provider = 27`，并显示 `other_fact` 主导 `70` 个失败 relation、`suggest_new_ideas` 是最弱 question type。当前 targeted line 已把 original hard-4 的 actual runtime gate 提回 `4/4`，corrected local-only `smoke8`、partial `smoke16`、两组 focused `suggest_new_ideas` slice 与 fresh mixed partial gate 都给出了持续正向的 local measurement，而 `49e1841` 又进一步说明 `belief=true` 这类 boolean-like projection noise 可以用 generic support-slot backfill 机械修掉。因此 blocker 已不再是“这条值噪声线本身能否成立”，而是“把 mixed Persona local-only gate 继续往后推进后，这条修复是否真的能把 broader local ceiling 从旧的 `5/8` 往上推；如果不能，下一条更值钱的改动就应回到 selected-slot ranking / larger local-only measurement”。`v33` 继续是 active measurement / refine，而不是 keep closeout。
