...
- Top next action: `TD-054 / WS-040 / v6.5` 当前 active line 的 latest runtime truth 已恢复到 `needed_facet_missing = 434`、PersonaMem no-routing `150/589`、verifier `85`。iteration `7 discard` 又排除了简单的 option-conditioned decision readout：它会把 full589 打回 `142/589`、把 `needed_facet_missing` 恶化到 `441`。下一步若继续推进，必须改更广的 question-conditioned reader / decision supervision，而不是继续做 per-option reread 或单条 feature 接线。
