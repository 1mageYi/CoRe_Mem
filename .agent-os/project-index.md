...
- Top next action: `TD-044 / WS-030` 当前仍是 stage-2 主线。当前 retained `v33` 仍是 `36/47 keep`；commit `2d3e59c` 已把 provider raw-output repair 接进 canary runner / resume path，而当前 Persona learned-authoritative partial full-holdout 已推进到 `99/512`，provider/local 为 `18/99`、`42/99`。最新 layered analysis 说明主矛盾已经从纯 provider-contract 污染收窄为 `projection/local = 54` 与 `provider-only = 27`；本轮后续 exploration 已进一步收敛成五层 truth：其一，support/value hygiene 线仍然不够，grounded same-relation backfill 与 relation-consistent support cleanup 都没有形成外部 keep；其二，current HEAD `3234a4b` 已把 option scorer 收紧成“过滤低信息词 + 惩罚 unsupported 长细节”的 generic ranking，clean targeted Persona `4`-sample gate `outputs_v2/v33_option_penalty_targeted_clean/` 从 `0/4` 提到 `1/4`，且没有 previously-correct 样本退化；其三，preference-facet fast-path 单独不足以形成外部 keep；其四，current HEAD `65c2612` 把 software-centric music clauses 规范成稳定 `music_preference` facet 并保留 symbolic=`new` 的 preference-facet fast-path 后，clean targeted Persona `4`-sample gate `outputs_v2/v33_music_software_facet_local/artifacts/20260420T231900Z_compare.json` 又从 retained `1/4` 提到 `2/4`，其中 `5370...` 明确翻正；其五，current HEAD `2b54785` 已把 parser 里过宽的 `from`-location 规则收紧，令 `5370...` 在保持正确 `(b)` 的同时把 learned belief relation 从 `location` 修回 `music_preference`。下一步仍保留当前 `gpt-agent.cc/v1` 路径，但 focused hypothesis 现在应更具体地前推到 selected-slot ranking，尤其是 `acd742...` 与 `a40d5...` 这类 recall/reason case，因为 software facet 已开始回流，而 relation drift 里的显式 parser bug 已被切掉。
...
- Active blockers: `TD-044 / WS-030` 当前已不再是“必须先切 provider/interface 才能继续”的硬 blocker；current HEAD `2d3e59c` 证明在同一 `gpt-agent.cc/v1` 代理下，把 stored `provider_raw_prediction` 重新做协议层 `<think>` / fenced-reasoning 清洗，能把 Persona partial learned-authoritative holdout 从 `provider exact = 5/66` 回收到 `15/77`，而后续扩大到 `99/512` 时 provider/local 为 `18/99`、`42/99`。最新 analysis artifact 已把剩余失败拆成 `projection = 54`、`provider = 27`，并显示 `other_fact` 主导 `70` 个失败 relation、`suggest_new_ideas` 是最弱 question type。本轮又进一步确认：代表性 failures `acd742...` / `5370...` 的 top-1 selected slot 本身就是长抽象 `other_fact`；support cleanup 只能把 belief/support 清干净，option-scorer 收紧虽然把 targeted Persona `4`-sample 从 `0/4` 推到 `1/4`，而单独的 preference-facet fast-path 也没有额外 external gain。但 current HEAD `65c2612` 又说明 parser/write inventory 不是伪问题，而 current HEAD `2b54785` 进一步说明 relation drift 里至少有一部分只是 parser bug：`5370...` 的 learned belief 现已从错误的 `location` 修回 `music_preference`。因此当前更准确的 blocker 是：selected-slot ranking 仍弱于 retained `v32`，而剩余 belief relation instability 更像 downstream selection / decoding 问题而不再是那个过宽 `from` 规则本身。`v33` 继续是 active measurement / refine，而不是 keep closeout。
