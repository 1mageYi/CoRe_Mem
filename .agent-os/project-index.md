...
- `WS-038` `[doing]`: Stage-2 `v6.3 Recall-Preserving Confidence-Aware Write Policy`，目标是在保留 v6.1 reader/decision 与 v6.2 write-quality 主链的前提下，把 binary hard filter 升级成 non-collapsing memory policy。
  ...
  - Current negative truth: `latest_stage2_v63_personamem_no_routing.json` 记录 full589 no-routing `180/589`，text-only `180/589`，option-only `235/589`；`latest_stage2_v63_decision.json` 记录 `negative_result`
  - Latest discard: 更激进的 durable-fact core promotion 会把 bank 推到 `core=40 / residual=435`，但 full589 no-routing 掉到 `152/589`；而 selected-support-gloss overlap decision-feature trial 会把 full589 no-routing 从 retained `180/589` 打坏到 `173/589`；两条线都已被 `git revert --no-edit` 回滚
  - Current next action: preserve the current non-collapse write-policy line and improve support selection / belief quality instead of pure decision-only gloss features; do not regress to tiny clean banks, answer-time routing, or raw-context retrieval
