...
- Top next action: `TD-052 / WS-038 / v6.3` 当前 managed run 已触发第三次 `pivot`，进入 protocol-defined soft blocker handoff。最新两条 search-derived structural trials“relation-pooled belief composer”“late-interaction reader features”都把 internal metrics 拉高，却分别把 full589 no-routing 打到 `160/589`、`156/589`；当前 restore line 只回到 `180/589`，仅打平 text-only `180/589` 且继续落后 option-only `235/589`。在当前 scope 内，下一步不再是继续追加 readout tweak，而是等待更广的 redesign / human reframing。
