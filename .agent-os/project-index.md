...
- Current v32 holdout truth: clean authoritative `timeout45` runs 已完成并发布。`outputs_v2/runs/v32_full_longmemeval_500_timeout45/` 当前固定为 `provider/local = 21/14`，相对 retained `v30` 的 `19/14` 至少 provider 侧为正；`outputs_v2/runs/v32_full_personamem_512_timeout45/` 当前固定为 `provider/local = 183/175`，其 published full-holdout compare 已确认 `personamem_gain_confirmed = true`。
- Current v32 clean-holdout truth: current HEAD `34943df` 的 resumable provider commit + run-dir lock 是这轮 full holdout 能收口为 authoritative evidence 的关键 runtime 修复；随后 `7377c29` 的 timeout/backoff 配置支撑了两条 clean run 跑完。由于 `run_stage2_memory_canary.py` 的统一 `v32_stage2_memory_canary.json` summary alias 会被后续 benchmark 覆盖，本轮额外保留了 benchmark-specific full summaries 并据此发布 authoritative `latest_*_stage2_v32_full.json`。
- Active blockers: 当前 `TD-043 / WS-029` 没有 runtime blocker；这轮 stop condition 已满足
