...
- `WS-038` `[doing]`: Stage-2 `v6.3 Recall-Preserving Confidence-Aware Write Policy`，目标是在保留 v6.1 reader/decision 与 v6.2 write-quality 主链的前提下，把 binary hard filter 升级成 non-collapsing memory policy。
  ...
  - Current reason for launch: `v6.2` already proved learned write-quality can enter the authoritative path, but it collapsed the bank to `core=4 / residual=42 / writes=64` and regressed PersonaMem to `160/589`. The next run must preserve recall rather than delete memory.
