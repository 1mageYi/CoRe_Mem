...
- Top next action: `TD-054 / WS-040 / v6.5` 当前 current HEAD 已进入 iteration `3 refine`：facet-type glossing + salient phrase selection 让 selected slots 从 `music_preference=pressured pressured` 一类噪声显著收敛到 `preference_target=... / environment_aversion=... / update_reason=...` 的 facet语义，并把 `needed_facet_missing` 压到 `434`、把 PersonaMem no-routing 提到 `150/589`，但 verifier 仍停在 `85`。下一步必须直接修 question-conditioned reader / decision 对这些更干净 facet glosses 的消费，而不是回到 token cleanup。
