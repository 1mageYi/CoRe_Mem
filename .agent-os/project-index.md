...
- Active blockers: `TD-044 / WS-030` 当前已不再是“必须先切 provider/interface 才能继续”的硬 blocker；current HEAD `2d3e59c` 证明在同一 `gpt-agent.cc/v1` 代理下，把 stored `provider_raw_prediction` 重新做协议层 `<think>` / fenced-reasoning 清洗，能把 Persona partial learned-authoritative holdout 从 `provider exact = 5/66` 回收到 `15/77`，而后续扩大到 `99/512` 时 provider/local 为 `18/99`、`42/99`。最新 analysis artifact 已把剩余失败拆成 `projection = 54`、`provider = 27`，并显示 `other_fact` 主导 `70` 个失败 relation、`suggest_new_ideas` 是最弱 question type。当前 targeted line 已把原始 hard-4 里的已知 blocker 逐条打通：`5370...` 通过 software facet 注入 + parser drift 修复翻正，`a40d5...` 通过 feedback-reason coverage 翻正，`acd742...` 现又通过 `eade2c3` 的 latent-facet rerank + belief-coercion 修复翻正。因此当前更准确的 blocker 已从“剩余唯一 hard sample 未解”变成“official verifier 仍是 `36`，且 full Persona context 在 learned slot-assignment runtime 上做 hard-4 回归过慢，不适合作为快速 gate”。也就是说，当前下一步主问题是建立一个更便宜的 non-regression measurement，再决定是否重启更大 Persona partial holdout；`v33` 继续是 active measurement / refine，而不是 keep closeout。
