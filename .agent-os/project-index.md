...
- Top next action: `TD-054 / WS-040 / v6.5` 当前 active line 的 latest runtime truth 已恢复到 `needed_facet_missing = 434`、PersonaMem no-routing `150/589`、verifier `85`。iteration `8 discard` 又排除了简单的 intent-conditioned facet reranking：它会把 full589 打到 `147/589`、把 `needed_facet_missing` 恶化到 `436`。当前下一步不再继续改 slot-selection heuristics，而是进入第二次 pivot 后的新 family：优先做 question-type / facet-type supervision 或 relevance+novelty 的 MMR-style shortlist，直接约束问句消费哪些 facet。
- Active workstreams: `WS-040`
- Active workstream label: `TD-054 / WS-040`
- Active workstream version: `v6.5`
- Current v6.5 refine truth: current HEAD `54771d1` 已在回滚 iteration `7 discard` 后重发 latest artifacts；materialized `canonical_gloss` 继续保持 `facet_type=value`，`preference_target` / `generic_fact` 继续使用 salient phrase。`latest_stage2_v65_error_attribution.json` 当前为 `needed_facet_missing_count = 434 > parser_only 409`；`latest_stage2_v65_personamem_no_routing.json` 当前为 full589 no-routing `150/589` vs text-only `234/589` vs option-only `235/589`；当前 `selected_slot_glosses` 已不再被 relation-level boilerplate 主导，但 verifier 仍是 `85`，所以这是 active refine line，不是 retained keep。
- Current v6.5 retained truth: current HEAD 已新增 `src/core_mem/v2/v65_facetized_memory.py`、`scripts/publish_stage2_v65_facetized_memory.py`，并把 `PersistentCoreResidualMemory` 的 same-relation match 升级成 `facet_match_key` matching。`latest_stage2_v65_facetized_memory_eval.json` 记录 full-context `37` shared contexts 下的 authoritative facetizer、`8` 类 facet schema、`facet_rerouted_actions = 815`；`latest_stage2_v65_persistent_state.json` 记录 non-collapse `core_bank=647 / residual_bank=1675 / writes=3492`；`latest_stage2_v65_error_attribution.json` 记录 facet-level buckets 已成立，但 `needed_facet_missing_count = 464 > parser_only 409`；`latest_stage2_v65_personamem_no_routing.json` 记录 full589 no-routing `123/589` vs text-only `218/589` vs option-only `235/589`，因此当前是 partial keep / negative_result，而不是 closeout。
- Latest v6.5 discard truth: `research-results.tsv` iteration `2 discard` 已记录 cue-anchored phrase retention 试验。该 trial 会把 `needed_facet_missing` 改善到 `448`、把 PersonaMem full589 no-routing 提到 `138/589`，但 `scripts/verify_stage2_v65_facetized_memory.py --score-only` 仍停在 `85`，因此不能进入 retained state；当前代码线已按 rollback policy 回到 retained partial，并重发 `latest_stage2_v65_*` 产物。
- Latest v6.5 discard truth (iteration `4`): 直接把 decision features 接到 `selected slot canonical_gloss` 只会让 `needed_facet_missing` 从 `434` 轻微挪到 `433`，PersonaMem no-routing 则停在 `150/589`，verifier 仍是 `85`。该 patch 已回滚，说明当前缺口不是单条 feature 接线，而是 decision supervision / objective 仍不够贴近真实 PersonaMem question-option 消费。
- Latest v6.5 discard truth (iteration `5`): 把 reader 侧对齐信号扩到 facet metadata 与 environment-query semantics 会把 full589 no-routing 直接打回 `128/589`，并把 `needed_facet_missing` 恶化到 `456`，verifier 仍是 `85`。该 patch 已回滚，说明继续沿 local reader-feature surgery 细抠的预期收益很低。
- Current pivot truth: `research-results.tsv` iteration `6 pivot` 已正式关闭当前 local feature surgery family。下一条实验族必须改 broader question-conditioned reader / decision supervision，而不是继续叠加 canonicalization / reader-feature / decision-feature 小修补。
- Latest v6.5 discard truth (iteration `7`): option-conditioned decision readout 虽然把 v61 head 改成 base-question readout + option-conditioned reread 联合训练/推理，但 full-context publish 反而把 PersonaMem full589 no-routing 从 `150/589` 打回 `142/589`，并把 `needed_facet_missing` 从 `434` 恶化到 `441`，verifier 仍是 `85`。该 patch 已回滚，说明当前缺口也不是“每个选项各读一次 memory”就能补齐。
- Latest v6.5 discard truth (iteration `8`): intent-conditioned facet readout 把 query-intent facet priors 与 duplicate-facet crowding penalty 接进 v61 readout selection，但 full-context publish 仍把 PersonaMem full589 no-routing 从 `150/589` 打回 `147/589`，并把 `needed_facet_missing` 从 `434` 恶化到 `436`，verifier 仍是 `85`。该 patch 已回滚，说明继续沿 hand-tuned slot-selection heuristics 细抠也不是 gain driver。
- Current pivot truth (iteration `9`): `research-results.tsv` iteration `9 pivot` 已正式关闭“broader supervision 但仍直接改 slot selection”的子 family。naive option-conditioned reread 与 intent-conditioned facet reranking 都已在 authoritative full-context publish 上失败，下一条实验族必须更直接地约束 question-conditioned answer projection / facet-type supervision，而不是继续堆 selection heuristic。
- Latest search truth (iteration `10`): `research-results.tsv` iteration `10 search` 已记录两条主来源启发：一是 Gao et al. 2019 的 DST-as-reading-comprehension，把 retrieval 看成 question-conditioned slot QA；二是 Carbonell & Goldstein 的 MMR，把 shortlist 看成 relevance + novelty 的组合优化。当前最合理的下一步是把这两条启发收敛成更 principled 的 question-to-facet supervision 或 diversified shortlist，而不是继续做局部 query prior。
