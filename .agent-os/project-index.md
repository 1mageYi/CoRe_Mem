...
- Top next action: `TD-054 / WS-040 / v6.5` 当前 retained partial 仍是 full-context authoritative `v6.5`：explicit facet schema、authoritative facetizer、facet-aware write utility 与 facet-level error attribution 已真正接进 memory path，verifier 继续维持在 `85`。最新 iteration `2 discard` 证明，单靠 cue-anchored phrase retention 虽能把 `needed_facet_missing` 从 `465` 压到 `448`、把 PersonaMem no-routing 从 `122/589` 提到 `138/589`，但仍不能抬高主分数；下一步必须直接做 facet-level support retention / value selection，而不是继续停留在 token-level canonicalization 去重。
