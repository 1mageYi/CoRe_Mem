...
- Top next action: `TD-044 / WS-030` 当前仍是 stage-2 主线。当前 retained `v33` 仍是 `36/47 keep`；commit `2d3e59c` 已把 provider raw-output repair 接进 canary runner / resume path，而当前 Persona learned-authoritative partial full-holdout 已推进到 `99/512`，provider/local 为 `18/99`、`42/99`。最新 layered analysis 说明主矛盾已经从纯 provider-contract 污染收窄为 `projection/local = 54` 与 `provider-only = 27`；本轮后续 exploration 已进一步收敛成三层 truth：其一，support/value hygiene 线仍然不够，grounded same-relation backfill 与 relation-consistent support cleanup 都没有形成外部 keep；其二，current HEAD `3234a4b` 已把 option scorer 收紧成“过滤低信息词 + 惩罚 unsupported 长细节”的 generic ranking，clean targeted Persona `4`-sample gate `outputs_v2/v33_option_penalty_targeted_clean/` 从 `0/4` 提到 `1/4`，且没有 previously-correct 样本退化；其三，preference-facet fast-path 虽然能保住多条 active `music_preference` 槽位，但 targeted Persona `4`-sample gate 仍停在 `1/4`，说明 write-path merge 不是当前 external gain driver。下一步仍保留当前 `gpt-agent.cc/v1` 路径，但 focused hypothesis 现在应更具体地前推到 selected-slot ranking，尤其是 `acd742...` 这类 recall/reason case，因为 option head 已开始产生小幅正增益，而 top-1 abstract slot 误选仍未解。
