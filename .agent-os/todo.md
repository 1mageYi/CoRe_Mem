...
- `TD-054` `[doing]` 以 `v6.5 Facetized Observation-to-Memory Redesign` 为目标，把 clause-level observation / slot memory 升级成 facet-structured latent memory unit。
  ...
  - Current partial: `latest_stage2_v65_facetized_memory_eval.json` 记录 authoritative facetizer、`8` 类 facet schema、`facet_candidate_count = 4834`、`facet_rerouted_actions = 1014`；`latest_stage2_v65_persistent_state.json` 记录 non-collapse `core=691 / residual=1732 / writes=3776`
  ...
  - Latest refine: `research-results.tsv` iteration `3 refine` 说明 facet-type glossing + salient phrase selection 已显著清理 selected slots，并把 `needed_facet_missing` 压到 `434`、把 no-routing 提到 `150/589`；当前 latest restore 已重新回到这条 refine truth，但 retained metric 仍是 `85`
