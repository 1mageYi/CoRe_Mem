...
- `TD-052` `[doing]` 以 `v6.3 Recall-Preserving Confidence-Aware Write Policy` 为目标，把 v6.2 的 hard write filter 升级成真正的 confidence-aware memory policy。
  ...
  - Latest discard: the durable-fact core-promotion trial degraded full589 no-routing to `152/589`; the selected-support-gloss decision-feature trial degraded it to `173/589`; the state-anchored support-supervision expansion degraded it to `144/589`; the scenario-aware reader-semantics trial degraded it to `162/589`; the question-conditioned belief-selector trial degraded it to `176/589`; the query-relation-router trial degraded it to `163/589`; the relation-pooled belief-composer trial degraded it to `160/589`. All seven lines have been reverted and should not be retried without a different strategy family.
  - Current top next action: `[SEARCH]` preserve the current non-collapse write-policy line and stop iterating inside both the selected-slot rerank / routing family and the first pooled-belief-composition variant. The next hypothesis must change belief representation / projection more fundamentally than slot reordering or relation pooling, while still forbidding tiny clean banks, answer-time routing, and raw-context retrieval.
